...
function CommandHint({ children }: { children: ReactNode }) {
	return (
...
async function invokeDocumentAction(
...
function CommandPaletteLayer({
	...
	const viewItems = [
		{
			...
			subtitle: '进入主文档空间视图',
			meta:
				pathname === '/docs' && !searchParams.get('view') ? '当前' : undefined,
					...
					? [
							{
								id: 'current-favorite',
								section: '当前文档',
								title: currentDocument.isFavorite
									? '取消收藏当前文档'
									: '收藏当前文档',
								subtitle: currentDocument.isFavorite
									? '从收藏中移除'
									: '加入收藏列表',
					...
					? [
							{
								id: 'current-archive',
								section: '当前文档',
								title: currentDocument.isArchived
									? '恢复当前文档'
									: '归档当前文档',
								subtitle: currentDocument.isArchived
									? '让页面重新出现在主导航中'
									: '从主导航隐藏，但保留内容',
	...
	const documentItems = (
		deferredQuery
			? indexedDocuments.filter((item) =>
					matchesQuery(item.searchText, deferredQuery),
				)
		...
		>
			<div className='flex flex-col'>
				...
				<div className='flex items-center justify-between border-t border-black/6 px-4 py-3 text-xs text-muted sm:px-5'>
					<span>
						{isPending ? '正在执行命令...' : '输入关键词即可筛选文档与操作'}
					</span>
...
export function CommandPaletteProvider({
	...
	const [currentDocument, setCurrentDocument] =
		useState<CurrentDocumentContext | null>(null);
