	...
	},
	{
		...
		icon: (active) => (
			<svg
				viewBox='0 0 20 20'
				fill='none'
				className={cn('h-4 w-4', active && 'scale-105')}
			>
				<path
...
function getParticipantInitials(
	participant: Pick<CollabUser, 'name' | 'email'>,
) {
...
function EditorSurface({
	...
	const onlineParticipants =
		participants.length > 0 ? participants : [bootstrap.user];
	...
	return (
		<div className='editor-frame flex min-h-[calc(100vh-8.5rem)] flex-col overflow-visible rounded-none shadow-none'>
			...
			<div className='flex items-center justify-between gap-3 bg-white px-5 py-2.5 text-xs text-muted sm:px-8 lg:px-12'>
				<span>
					{metrics.words} 词<span className='mx-2 text-black/16'>/</span>
		...
		async function boot() {
			try {
				...
				const response = await fetch(
					`/api/documents/${documentId}/collab-token${search}`,
					{
						cache: 'no-store',
					},
				);
